...
public class UploadVideoService extends IntentService{
	...
	public void onCreate() {
		super.onCreate();
		Log.v(TAG, "UploadVideoService onCreate called");
					   ...
					   // Log.v(TAG, "SeekBar values is: "+seekbarprogress);
					    ...
					    CustomMultiPartEntity entity=new CustomMultiPartEntity(yourEntity, new ProgressListener() {
							...
							public void transferred(long num) {
								...
								// ImageUtil.galleryLog(TAG, "publish progress :"+(int) ((num / (float) totalsizeimage) * 100));
							}
						...
						if (resp.equalsIgnoreCase("Successfully Uploaded Video File")) {
							...
							for (int i = 0; i < allpath.length; i++) {
							 ...
							 NotificationManager notificationManager = (NotificationManager)getApplicationContext()
							  ...
							  //  stopSelf(currentId);  // stop service once it done
				...
				try {
					...
					String url=ImageConstant.BASEURL+"video_upload.php";
					   ...
					   // Log.v(TAG, "total size is: "+totalsize);
					    ...
					    ImageUtil.galleryLog(TAG, "Json response is: "+response1);
